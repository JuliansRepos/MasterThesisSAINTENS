...
from auxiliary.data import DataModule
from optim import DelayedScheduler, Lamb, Lookahead
        ...
        for feature_index, start, end in zip(range(len(self.__feature_boundaries[0])), *self.__feature_boundaries):
            ...
            if feature_index in self.__feature_discrete:
                ...
                if feature_index in self.__target_discrete:
                    ...
                    accuracy = roc_auc_score(np.array(target.detach().argmax(dim=1).cpu()),np.array(torch.softmax(prediction.detach(),dim=1).detach().cpu()[:,1]))
