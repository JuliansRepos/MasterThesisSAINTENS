...
def catmix(x_categ,x_cont, lam, na_idx):
    device = x_categ.device
    batch_size = x_categ.size()[0]
    index = torch.randperm(batch_size)
    cat_corr = torch.from_numpy(np.random.choice(3,(x_categ.shape),p=[lam/2,lam/2,1-lam])).to(device)
    con_corr = torch.from_numpy(np.random.choice(2,(x_cont.shape),p=[lam,1-lam])).to(device)
    x1, x2 =  x_categ[index,:], x_cont[index,:]
    x_categ_corr, x_cont_corr = x_categ.clone().detach() ,x_cont.clone().detach()
    x_categ_corr[cat_corr==0] = x1[cat_corr==0]
    na_idx = np.array(na_idx.copy()).reshape((1,-1))
    na_idx = np.tile(na_idx, reps=(x_categ_corr.shape[0],1))
    na_idx = torch.tensor(na_idx, dtype=x_categ_corr.dtype, device=device)
    x_categ_corr[cat_corr == 1] = na_idx[cat_corr == 1]
    x_cont_corr[con_corr==0] = x2[con_corr==0]
    cat_corr[cat_corr==0]=1
    cat_corr[cat_corr==2]=0
    con_corr = 1-con_corr
    cat_corr=cat_corr.to(dtype=torch.bool)
    con_corr=con_corr.to(dtype=torch.bool)
    return x_categ_corr, x_cont_corr, cat_corr, con_corr
